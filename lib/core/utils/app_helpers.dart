...
class AppHelpers {
  static String translate(String s, [bool isArabic = true]) =>
      isArabic ? Get.translations['ar']![s] ?? s : s;
  ...
  static Widget appDivider([
    ...
    color: AppColors.appMainColor.withOpacity(opacity),
    thickness: thickness,
    height: height,
  );

  static Map<String, T> listToIDMap<T>(list) {
  ...
  static Future<bool> mLaunchWhatsApp(
    String num, [
    String msg = 'Hello',
  ]) async {
  ...
  static void showDeleteAccountDialog(UserModel userModel) {
    RxInt timeTicks =
        (AppController.to.isAdmin
                ? 5
                : AppConstants.deleteAccountCautionSeconds)
            .obs;
          ...
          (_) => SuperDecoratedContainer(
            ...
            child: Column(
              ...
              children: [
                const IconButton(
                  onPressed: mHide,
                  icon: Icon(Icons.dangerous, color: Colors.red, size: 40),
                ),
                Padding(
                  padding: const EdgeInsets.all(16),
                  child: Txt(
                    '${"Are you sure you want to ".tr}${AppStrings.deleteAccMsgEn.tr}',
                    fontSize: 18,
