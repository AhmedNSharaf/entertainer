...
class SuperOSMController {
  ...
  LatLng get currentPositionReversed =>
  ...
  LocationModel? get selectedLocationForTooltip =>
      _selectedLocationForTooltip.value;

  set selectedLocationForTooltip(LocationModel? val) => {
    _selectedLocationForTooltip.value = val,
    _selectedLocationForTooltip.refresh(),
  };
  ...
  initAll() async {
    ...
    if (SuperOSMManager.to.gpsPosition != null) {
      gpsPosition = LatLng(
        SuperOSMManager.to.gpsPosition!.latitude,
        SuperOSMManager.to.gpsPosition!.longitude,
      );
    }
  ...
  Future<LocationModel?> getPointDetails(
    LatLng geoPoint, [
    String? locale,
  ]) async {
