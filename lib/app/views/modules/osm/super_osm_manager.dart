...
class SuperOSMManager extends GetxService {
  ...
  List<LatLng> generateNeighboringLocations({
    ...
    for (int i = 0; i < numberOfPoints; i++) {
      ...
      double newLat = asin(
        sin(originLat) * cos(distance) +
            cos(originLat) * sin(distance) * cos(bearing),
      );
      double newLng =
          originLng +
          atan2(
            sin(bearing) * sin(distance) * cos(originLat),
  ...
  initAll() async {
    gpsPosition = GeoPoint(
      latitude: 32.55670430051541,
      longitude: 35.84727275554576,
    );
    GeoPoint? geoPoint = await getCurrentPosition(
  ...
  static Future<bool> addAbuZeitLocation(
    LocationModel locationModel, {
    File? imgFile,
    String? description,
  }) async {
  ...
  static Future<String?> uploadAbuZeitImage(File file, [String? loc]) async {
    ...
    try {
      var request = http.MultipartRequest(
        "POST",
        Uri.parse('https://ultimate.abuzeit.com/files'),
      );
...
MarkerIcon pickMarker = MarkerIcon(
  assetMarker: AssetMarker(image: const AssetImage(AppAssets.neuss)),
);
...
MarkerIcon toMarker = MarkerIcon(
  assetMarker: AssetMarker(
    image: const AssetImage(AppAssets.neuss),
    scaleAssetImage: 0.7,
  ),
);

MarkerIcon gpsMarker = const MarkerIcon(
  icon: Icon(Icons.person_pin_circle_sharp, color: Colors.indigo, size: 96),
);
